...
class Tracker(SLAMParameters):
    ...
    def tracking(self):
        ...
        for ii in range(self.num_images):
            ...
            if (self.iteration_images % 3 == 0 or self.iteration_images >= self.num_images-2):
                self.trajmanager.plot_traj(self.iteration_images, self.poses)
        ...
        self.write_output_txt(self.poses)
    ...
    def write_output_txt(self, est_poses):
        est_poses_flatten = np.reshape(est_poses, (-1, 16))
        
        output_pose_txt = os.path.join(self.output_path, "est_poses.txt")
        np.savetxt(output_pose_txt, est_poses_flatten)
